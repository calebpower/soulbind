...
| 9 | Simulated users | **Trimmed tier complete; one gate clause outstanding.** Oracle self-test, generator, actors, checker, executor, committed seeds and runner all landed, wired as the `sim` stage on both backends. The acceptance test PASSES on the workstation: control clean, and a reverted `LinkingService` fix rediscovered at action 50. Outstanding: three seeds × both backends in a session. Shrinker and two nemesis classes deferred — departure 9, DECISIONS 9.1 |
