...
| 8 | connector-plan + full-stack battery | **In progress — both gate clauses demonstrated**, battery green on both backends including the latin1 axis (asserted, not assumed), the PHP and shell mutation tiers, and a new `fuzz` stage driving hostile input at a deployment the journeys and sim have populated. Outstanding: the `browser` stage against the real stack, and 2 of the 3 named journeys |
