...
| 8 | connector-plan + full-stack battery | **In progress — both gate clauses demonstrated**, and the battery is green on both backends including the latin1 charset axis, the PHP mutation tier and the shell mutation battery. Outstanding: T6's astral-plane pushes and no-backdoor state building, T7 and T8 as stages, T5 against the real stack, and 2 of the 3 named journeys |
| 9 | Simulated users | **Trimmed tier complete; gate met.** Run 8 green on both backends, three fixed seeds × 400 actions each, 0 violations. The acceptance test passes on the workstation: control clean, reverted `LinkingService` fix rediscovered. Shrinker and two nemesis classes deferred (departure 9). **One open lead:** `decisions-follow-the-rules` fires against unmodified core and is excluded pending diagnosis — DECISIONS 9.10 |
