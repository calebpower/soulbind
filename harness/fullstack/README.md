...
SOULBIND_DB=mariadb ./run.sh up migrate journeys sim plan measures fuzz down
```
...
| `groups` | The proxy's group effector, end to end: a player linked through the real flow, core emitting `subject.requirements-met`, the connector's drain picking it up, and **LuckPerms holding the group** — read from LuckPerms' own JSON storage rather than a console reply, because the file is the authority. This is the stage that would have caught DECISIONS 10.23, where the effector had never been connected to anything and no tier could notice because no permissions plugin was in the stack. Runs on **both** axes: LuckPerms is configured with JSON storage and needs no database, unlike `plan`. |
| `t10` | Tier 10: deep reads over the world the sim just accumulated — the whole audit log paged past the 1000-row single-query ceiling in small pages, contiguity asserted, a greedy one-request query required to admit it was truncated, and filtered paging required to return exactly the rows a real `audit.push` principal contributed. Watchdog: any 5xx fails the stage, and no fault injection runs here — a 5xx is core failing under legitimate read load. Runs **before** `fuzz` so that blame stays unambiguous. |
...
| `measures` | A measurement taken on **one** platform must move the gate for the subject's identities on the **others**. Reports the dashboard's activity index for a linked game account and asserts core emits `requirements-met` for a sibling identity of a different kind, not only the one measured. This is the stage that would have caught the 0.2.1 defect: `measure.report` expanded its transitions to the measured identity alone, so a game-side measurement could never move a chat-side role — and reverting the fix left this entire tier green while turning exactly one unit test red, because the reporter was never enabled here. Asserts on the event stream rather than on a chat surface: this tier drives the chat connector through a CLI and runs no live chat effector, and the effector's half is covered against the scripted surface in `RoleEffectorTest`. Threshold deliberately zero — Tier 1 owns the arithmetic, and zero makes the transition fire when the first observation lands rather than depending on session data this stage does not control. **mariadb axis only**, for the same reason as `plan`. |
