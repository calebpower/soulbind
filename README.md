...
- **Not a metrics store.** It holds the latest observation a connector reports
  about an account, and never a history of them. Windows are computed by
  whatever already measures the thing; core compares one number and keeps no
  series.
...
| 11 | §14 Phase 10 | `core` and `connector-discord` ship as **distributions** — `bin/` plus `lib/` — rather than as fat JARs. The plugin JARs are shaded as written | §16 forbids an LGPL artifact inside a shaded artifact, and in this layout every dependency is already its own file, so the rule holds **by construction**. In a fat jar it would hold by an exclusion list, which is a thing that can be silently wrong in the direction of a licence violation — and the licence inventory found two copyleft artifacts (JNA via JDA's voice support, trove4j) that nobody had noticed were in the graph at all. Second, Javalin, Jetty, Flyway and the JDBC drivers all register through `META-INF/services`; merging those wrongly drops a services file rather than failing the build, and the symptom is "no suitable driver" on the operator's machine. Against that there is no operator benefit: a systemd unit runs a start script either way, and this is already the layout the full-stack and forum tiers have run core from since Phase 7. `ServiceDistGuardTest` asserts the property the departure is justified by — that every artifact the generated inventory calls unbundled really is its own replaceable jar in `lib/`, on an explicit classpath. `connector-velocity` and `connector-plan` are shaded single files as §14 says, because a host loads one jar out of `plugins/` and there is no `lib/` to unbundle into |
...
| 12 | §8, §6 | The evaluator takes a **fourth class of input** — *measures*: named numeric observations a connector reports about an identity, carrying the window they cover and when they were taken. §8 enumerates evaluation's inputs as `(identity graph slice, rules, overrides, clock)`, and §6's data model lists no such table | The **purity** §8 exists to guarantee is untouched, and that is the whole of what it was protecting: `PolicyEngine.decide` still performs no I/O, holds no clock of its own, and is still reproducible from its arguments, so the Tier 4 matrix stays exhaustive rather than representative. What changed is the *enumeration*, and the enumeration was written when the only questions a gate could ask were about identity. "Verified, linked, and has actually been here" is the same kind of question — a predicate over things core is told, evaluated against a clock it is handed — and expressing it anywhere else puts a policy decision somewhere core cannot audit it, which is the property §9 exists to keep. The departure is the input list and the one table behind it, and nothing else: core stores **one observation per (identity, name), overwritten**, performs no aggregation over time, keeps no history, and runs no sweep. A connector reports an answer it already computed, and core stores it with enough provenance to decide whether to still believe it |
